...
if len(judgeArgs) < 9:
    print(
        f"!;0;1;0;0;Not Enough info to judge\nexpected 9 args got {len(judgeArgs)} args",
        end="",
    )
    exit(0)
...
def writeLog(text: str):
    with open(
        path.join(PROBLEM_DIR, f"{int(time.time())}T{testCase}LOG.txt"), "w"
    ) as f:
        f.write(text)
...
def variableCompare(data):
    ...
    if (
        VariableStuff.compareVar(data[2], ANS[tc][2], tol)
        and data[3].lower() == ANS[tc][3].lower()
    ):
        return "P", 1, 1, "Wow za!"
    else:
...
def compare():
        ...
        if data[1] == ANS[tc][1]:
            ...
            if ANS[tc][0] != data[0]:
                return (
                    ...
                    0,
                    1,
                    f"{ANS[tc][1]} : Expected {ANS[tc][0] == 'E' and 'Equation' or 'Number'} got {data[0] == 'E' and 'Equation' or 'Number'}",
                )
    ...
    return "X", 0, 1, f"{ANS[tc][1]} not found or Invalid format... :("
